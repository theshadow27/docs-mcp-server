...
describe("DocumentProcessingPipeline", () => {
  ...
  it("should process documents successfully", async () => {
    ...
    expect(mockScrape).toHaveBeenCalledWith(mockScrapeOptions, expect.any(Function));
    expect(onProgress).toHaveBeenCalledWith(mockProgress);
    expect(mockAddDocument).toHaveBeenCalledWith("test-lib", "1.0.0", expect.any(Object));
  ...
  it("should prevent concurrent processing", async () => {
    ...
    await expect(pipeline.process(mockScrapeOptions)).rejects.toThrow(PipelineStateError);
    await firstProcess;
  });
  ...
  it("should handle scraping errors", async () => {
    ...
    await expect(pipeline.process(mockScrapeOptions)).rejects.toThrow(mockError);
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining("Pipeline failed"));
  });
